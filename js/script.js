const submitButton = document.querySelector(".add-task__button");
const inputTask = document.querySelector(".add-task__input");

const countTasksElements = document.querySelectorAll(".count-tasks-js");
const countCompletedTasksElement = document.querySelector(
  ".count-completed-tasks-js"
);

const tasksList = document.querySelector(".tasks-created__list");
...
function updateCompletedTaskCount() {
  countCompletedTasksElement.textContent = countCompletedTasks;
}

function toggleEmptyList() {
  const isEmpty = countTask === 0;
  tasksList.classList.toggle("visually-hidden", isEmpty);
  emptyList.classList.toggle("visually-hidden", !isEmpty);
}

function addTask(taskText) {
  const taskItem = document.createElement("li");
  taskItem.className = "tasks-created__item";

  taskItem.innerHTML = `
    <span class="tasks-created__emulator"></span>
      <p class="tasks-created__description"></p>
      <svg class="tasks-created__icon" width="13" height="14" viewBox="0 0 13 14" fill="none" xmlns="http://www.w3.org/2000/svg">
...
tasksList.addEventListener("click", (event) => {
  ...
  const isDeleteIcon = event.target.closest(".tasks-created__icon");
  const isCompleteToggle = event.target.classList.contains(
    "tasks-created__emulator"
  );
  ...
  if (isDeleteIcon) {
    ...
    emulator.classList.toggle("tasks-created__emulator--completed");
    emulator.nextElementSibling.classList.toggle(
      "tasks-created__completed-text"
    );
    countCompletedTasks += emulator.classList.contains(
      "tasks-created__emulator--completed"
    )
      ? 1
      : -1;
